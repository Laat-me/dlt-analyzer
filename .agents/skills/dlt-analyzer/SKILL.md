...
| 术语 | 定义 | 示例（上期开 05 20） |
|------|------|----------------------|
| **重号** | 本期号码 = 上一期某个原号 | 05、20 |
| **跨期邻号** | 本期号码 = 上一期某个号码 ±1（不含原号本身） | 04、06、19、21 |
| **同期连号** | 同一期内相邻开奖号（如 19、20 同时开出） | 与上期无关 |

- 跨期邻号与重号**互斥**：一个号码不能同时算两者。
- 实现口径：严格跨期邻号用 `algorithms.prev_neighbor_flags(radius=1)`（按集合排除上期全部原号，即使上期自身含连号也不计入邻号）；`neighbor_counts(radius=1, exclude_center=True)` 为宽松计数（上期含相邻原号时可能相互计入），两者在构建独立特征时以 `prev_neighbor_flags` 为准。
- 该特征是否有效必须以长窗口回测判断，不得把“最近几期常出现”当作预测优势。
...
### 算法变更记录与恢复规则（第 25 轮用户要求，必须遵守）

- **任何算法/公式/权重/选号流程/数据口径的修改，必须先写入 `CHANGELOG.md` 再实施**（记录模板见该文件）
- 每次变更必须包含：变更内容、原因、**变更前后指标对比**（900/100 窗口 + 1000 期段双口径）、恢复方法
- **保留旧版**：被替换的算法/参数保留为 `*_orig` 变体或依赖 git 历史，不直接删除
- 历史变更从 round-7 起已补记入 `CHANGELOG.md`（含 AI_U_wide 7%→3.2%→2.4-2.8%→还原版 8%/3.5% 的完整轨迹）
...
- 跨期邻号统计（近10期/近30期）：每期命中「上一期号码 ±1」的个数分布与频次，按 `prev_neighbor_flags(radius=1)` 统计，与重号分别计数（见术语定义）
...
- `freq5` = 近5期出现次数
- `freq10` = 近10期出现次数
- `neighborBonus` = 号码处于**上一期开奖号码 ±1 的跨期邻号**内（不含重号）-> +4
- `streakBonus` = 近5期出现 >= 2 次 -> +4

高分 = 近期频繁出现 + 处于上期号码邻域 + 连续出现。
