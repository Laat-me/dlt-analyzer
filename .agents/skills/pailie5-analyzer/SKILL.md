...
| 模型 | 核心策略 | 适用场景 |
|------|---------|---------|
| **m1 冷热折中** | gap×0.5 + freq5×1.5 + freq10×0.8 + (100-freqAll)×0.3 | 每位独立评分，兼顾遗漏与热度 |

`freqAll` 基于 1000 期样本：每位 10 个数字理论均出 100 次，故用 `(100-freqAll)` 衡量全期偏冷。
...
#### m1 评分（冷热折中，每位独立）
```
score[pos][d] = gap×0.5 + freq5×1.5 + freq10×0.8 + (100-freqAll)×0.3
```
